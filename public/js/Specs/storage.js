...
function addStorageEventListeners() {
  ...
  let deleters = document.getElementsByClassName("stDelete");
...
function sendItemCreateRequest(event) {
  ...
  let value = this.querySelector("input[name=inputStName]").value;
...
function itemCreateHandler() {
  if (this.status != 201) {
    ...
    alert("Failed to create storage :'(");
  }
  ...
  let table = document.querySelector("tbody.stTableBody");
...
function itemDeleteHandler() {
  ...
  let item = JSON.parse(this.responseText);
  let element = document.getElementById("st-" + item.id);
  element.remove();
}

function createItem(item) {
  let new_item = document.createElement("tr");
  new_item.classList.add("st");
  new_item.setAttribute("id", "st-" + item.id);
  new_item.innerHTML = `
                <td>${item.value}</td>
                    <td><a value="${item.id}" class="stDelete thumbnail">
                        <i class="far fa-times-circle fa-2x ml-4 text-danger"></i>
                </a> </td>
                `;

  new_item
    .querySelector("a.stDelete")
